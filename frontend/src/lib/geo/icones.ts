...
// certas direções.
export function motoDivIcon(heading: number | null, size = 36) {
  const html = `
    <div style="position:relative;width:${size}px;height:${size}px">
      ...
      <div style="width:${size}px;height:${size}px;border-radius:9999px;background:linear-gradient(135deg,#e08a3a,#d5aa45 55%,#b57c27);display:flex;align-items:center;justify-content:center;box-shadow:0 2px 8px rgba(0,0,0,.45);border:2px solid #fff">
        ${BIKE_SVG}
      </div>
    </div>
  `
...
export function destDivIcon(size = 30) {
  return L.divIcon({
    className: 'icone-limpo',
    html: `<div style="filter:drop-shadow(0 2px 3px rgba(0,0,0,.45))">${pinSvg(size)}</div>`,
    iconSize: [size, size],
