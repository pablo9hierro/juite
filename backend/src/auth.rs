...
impl FromRequestParts<AppState> for SunsetAdminSession {
    ...
    async fn from_request_parts(
        ...
        let subject: Option<(String,)> = sqlx::query_as(
            "SELECT subject_id FROM sunset.sessions WHERE token = $1 AND role = 'admin' AND expires_at > now()",
        )
        .bind(token)
...
impl FromRequestParts<AppState> for SunsetMotoboySession {
    ...
    async fn from_request_parts(
        ...
        let subject: Option<(String,)> = sqlx::query_as(
            "SELECT subject_id FROM sunset.sessions WHERE token = $1 AND role = 'motoboy' AND expires_at > now()",
        )
        .bind(token)
        .fetch_optional(&state.pool)
        .await
        ...
        match subject {
            Some((id,)) => Ok(SunsetMotoboySession(id)),
            None => Err(AppError::Unauthorized("invalid or expired session".to_string())),
        }
    }
}
